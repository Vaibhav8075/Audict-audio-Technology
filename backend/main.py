...
async def lifespan(app: FastAPI):
    ...
    shutdown_scheduler()
app = FastAPI(title='DCM Audit System API', description='AI-powered call audit management platform for DCM', version='1.0.0', lifespan=lifespan)
...
app.add_middleware(CORSMiddleware, allow_origins=os.getenv('ALLOWED_ORIGINS', 'http://localhost:5173,http://127.0.0.1:5173').split(','), allow_credentials=True, allow_methods=['*'], allow_headers=['*'])
