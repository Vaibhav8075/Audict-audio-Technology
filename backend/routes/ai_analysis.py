"""
AI Analysis Routes - Groq API Integration
Speech-to-text, sentiment analysis, summary generation, keyword extraction
"""
...
def ai_to_dict(ai: AIAnalysis) -> dict:
    return {
        "id": ai.id,
        "audit_id": ai.audit_id,
        "transcription": ai.transcription,
        "transcription_confidence": ai.transcription_confidence,
...
async def trigger_ai_analysis(
    ...
    if not recording or not recording.file_path:
        raise HTTPException(
            status_code=400,
            detail="No valid recording found for this audit"
        )
...
async def process_ai_analysis(audit_id: int, recording_path: str):
    ...
    try:
        ...
        # Step 2: Comprehensive analysis via Groq LLM
        ...
        ai_record = AIAnalysis(
            ...
            call_quality_score=analysis.get("call_quality_score"),
            customer_satisfaction_score=analysis.get("customer_satisfaction_score"),
...
async def get_ai_dashboard_stats(
    ...
    if current_user.role == UserRole.employee:
        analyses = db.query(AIAnalysis).join(Audit).filter(
            Audit.employee_id == current_user.id
        ).all()
    else:
    ...
    return {
        ...
        "top_issues": [{"issue": i[0], "count": i[1]} for i in issue_counts],
        "top_keywords": [{"keyword": k[0], "count": k[1]} for k in keyword_counts],
        "negative_calls": sentiments["negative"],
        "needs_attention": [
            {
                "audit_id": a.audit_id,
            ...
            for a in analyses
            if a.sentiment_label == SentimentLabel.negative
        ][:5]
    }
...
async def generate_feedback_suggestions(
    ...
    suggestions = await groq_service.generate_feedback_suggestions(
        analysis.call_summary or "",
        analysis.identified_issues or [],
        analysis.sentiment_label.value if analysis.sentiment_label else "neutral"
    )
