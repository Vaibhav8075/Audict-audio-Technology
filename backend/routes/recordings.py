"""
Recording Routes
Secure audio upload, streaming, and management.
...
async def upload_recording(
    ...
    if content_type not in ALLOWED_MIME_TYPES:
        raise HTTPException(
            status_code=400,
            detail=f"Invalid file type. Allowed: MP3, WAV, OGG, M4A"
        )
...
async def stream_recording(
    ...
    if not os.path.exists(recording.file_path):
        # File missing on disk but not marked expired - fix this
    ...
    # Handle HTTP Range Requests (required for audio seeking in browsers)
...
async def get_recording_info(
    ...
    return {
        "recording": {
            ...
            "is_expired": recording.is_expired,
            ...
            "has_file": recording.file_path is not None and not recording.is_expired
        }
    }
