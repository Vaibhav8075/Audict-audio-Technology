"""
Admin Routes
Full admin control: user management, system logs, analytics, exports
"""
...
from models import (
    User, Audit, Recording, FeedbackAnswer, AIAnalysis,
    AuditAccessLog, UserRole, AuditStatus, SentimentLabel
)
        ...
        "users": [
            {
                ...
                "audit_count": db.query(Audit).filter(Audit.employee_id == u.id).count()
            }
            for u in users
        ],
        "total": total
...
async def get_analytics_overview(
    ...
    # Weekly uploads
    weekly_uploads = db.query(Recording).filter(
        Recording.uploaded_at >= week_ago
    ).count()
    
    # This week's audits
    weekly_audits = db.query(Audit).filter(
        Audit.call_date >= week_ago
    ).count()
    ...
    # Sentiment breakdown from AI
    sentiment_counts = {}
    for s in SentimentLabel:
        sentiment_counts[s.value] = db.query(AIAnalysis).filter(
            AIAnalysis.sentiment_label == s
        ).count()
    ...
    for i in range(7):
        ...
        count = db.query(Audit).filter(
            Audit.call_date.between(day_start, day_end)
        ).count()
        daily_audits.append({
            "date": day.strftime("%a"),
            "count": count
    ...
    return {
        "summary": {
            ...
            "active_recordings": total_recordings,
...
async def get_access_logs(
    ...
    query = db.query(AuditAccessLog).options(
        joinedload(AuditAccessLog.user),
        joinedload(AuditAccessLog.audit)
    )
    ...
    return {
        "logs": [
            {
                "id": l.id,
                "audit_id": l.audit.audit_id if l.audit else None,
...
async def export_feedback_csv(
    ...
    # Header
    writer.writerow([
        "Submission ID", "Audit ID", "Client Name",
        "Employee", "Form", "Overall Rating",
    ...
    for s in submissions:
        writer.writerow([
            s.id,
            s.audit.audit_id if s.audit else "",
            s.audit.client_name if s.audit else "",
            s.submitted_by.full_name if s.submitted_by else "",
            s.form.title if s.form else "",
            s.overall_rating or "",
            s.comments or "",
