"""
Feedback Routes
Form creation, submission, and response management.
Feedback records are NEVER deleted per business requirements.
"""
...
from models import (
    User, Audit, FeedbackForm, FeedbackQuestion, FeedbackAnswer,
    FeedbackQuestionAnswer, FeedbackQuestionType, UserRole
)
...
class QuestionAnswerSubmit(BaseModel):
    question_id: int
    answer_value: Any  # Can be string, number, or list
...
async def get_forms(
    ...
    forms = db.query(FeedbackForm).options(
        joinedload(FeedbackForm.questions)
    ).filter(FeedbackForm.is_active == True).all()
...
async def get_form_detail(
    ...
    form = db.query(FeedbackForm).options(
        joinedload(FeedbackForm.questions)
    ).filter(FeedbackForm.id == form_id).first()
...
async def submit_feedback(
    ...
    existing = db.query(FeedbackAnswer).filter(
        FeedbackAnswer.audit_id == data.audit_id,
        FeedbackAnswer.form_id == data.form_id,
        FeedbackAnswer.submitted_by_id == current_user.id
    ).first()
    
    if existing:
        raise HTTPException(
            status_code=400,
            detail="You have already submitted feedback for this audit and form"
        )
    ...
    for answer in data.answers:
        qa = FeedbackQuestionAnswer(
            ...
            answer_value=str(answer.answer_value) if answer.answer_value is not None else None
        )
        db.add(qa)
...
async def get_my_submissions(
    ...
    submissions = db.query(FeedbackAnswer).options(
        joinedload(FeedbackAnswer.audit),
        joinedload(FeedbackAnswer.form),
        joinedload(FeedbackAnswer.question_answers).joinedload(FeedbackQuestionAnswer.question)
    ).filter(
        FeedbackAnswer.submitted_by_id == current_user.id
    ).order_by(desc(FeedbackAnswer.submitted_at)).all()
    ...
    for s in submissions:
        result.append({
            ...
            "audit": {
                ...
                "client_name": s.audit.client_name
...
async def get_all_submissions(
    ...
    query = db.query(FeedbackAnswer).options(
        joinedload(FeedbackAnswer.audit),
        joinedload(FeedbackAnswer.form),
        joinedload(FeedbackAnswer.submitted_by),
        joinedload(FeedbackAnswer.question_answers).joinedload(FeedbackQuestionAnswer.question)
    )
    ...
    for s in submissions:
        result.append({
            ...
            "form_title": s.form.title if s.form else None,
            "submitted_by": {
                "id": s.submitted_by.id,
                "full_name": s.submitted_by.full_name,
                "email": s.submitted_by.email
...
async def check_submission(
    ...
    existing = db.query(FeedbackAnswer).filter(
        FeedbackAnswer.audit_id == audit_id,
        FeedbackAnswer.form_id == form_id,
        FeedbackAnswer.submitted_by_id == current_user.id
    ).first()
